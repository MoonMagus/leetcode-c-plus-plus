...
public:
    ListNode* deleteDuplicates(ListNode* head) {
        ...
        while (head) {
            // Check current node val is equal with previous one or the next one.
            if ((prev == nullptr || head->val != prev->val) && (head->next == nullptr || head->val != head->next->val)) {
                rear->next = head;
                rear = head;
            }
...
int main() {
    ...
    Solution so;
    result.push_back(so.deleteDuplicates(list[0]));
    printList(result);
