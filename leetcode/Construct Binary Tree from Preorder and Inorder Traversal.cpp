...
private:
    ...
    TreeNode* buildTree(Iterator preBegin, Iterator preEnd, Iterator inBegin, Iterator inEnd) {
        ...
        Iterator root_pos = find(inBegin, inEnd, *preBegin);
        int left_size = distance(inBegin, root_pos);

        TreeNode* root = new TreeNode(*preBegin);
        root->left = buildTree(next(preBegin), next(preBegin, left_size + 1), inBegin, root_pos);
        root->right = buildTree(next(preBegin, left_size + 1), preEnd, next(root_pos), inEnd);
